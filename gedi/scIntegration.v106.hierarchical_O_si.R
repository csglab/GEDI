# ---> Need to fix solve.sigma to consider Ro
...
setRefClass(
  ...
  methods = list(
    ...
    solve.Rk = function( k ) {
      ...
      # Solve Rk
      lambda <- 1/hyperparams$S_Qi_mean/hyperparams$S_R
    ...
    solve.Ro = function() {
      ...
      # Solve Ro
      lambda <- 1/hyperparams$S_oi_mean/hyperparams$S_R
    ...
    solve.sigma2 = function()
    {
      ...
      for( i in 1:aux$numSamples ) # Add the sample-specific coefficients
      {
        if( aux$L==0 ) # matrix H is not provided
        {
          ...
          S <- S + vecL2_noPrior(params$oi[[i]]-aux$oi_hat[[i]]) / hyperparams$S_oi[i]
          S <- S + matL2_noPrior(params$Qi[[i]]-aux$Qi_hat[[i]]) / hyperparams$S_Qi[i]
        }
      ...
      if( aux$L > 0 ) {
        for( k in 1:aux$K ) {
          S <- S + matL2_noPrior(params$Rk[[k]]) / hyperparams$S_Qi_mean / hyperparams$S_R
        }
        S <- S + matL2_noPrior(params$Ro) / hyperparams$S_oi_mean / hyperparams$S_R
      }
      # Normalize sigma2
      params$sigma2 <<- S/N
      ...
      hyperparams$S_o <<- vecL2_wPrior(params$o,hyperparams$o_0) / params$sigma2 / aux$J
      tmp <- 0
      for( i in 1:aux$numSamples ) {
        tmp <- tmp + vecL2_wPrior(params$si[[i]],hyperparams$si_0[[i]])
      }
      hyperparams$S_si <<- tmp / params$sigma2 / aux$N
...
sourceCpp("./gedi/Rcpp/Eigen.v8.cpp")
...
# A simple function to retrieve the stored colData
...
# `Function to return the variance of the imputed Y (log-scale)
...
#######################
# Function to return the GEDI DB or ZDB projection (i.e., the DxB or the ZxDxB matrix)
#######################
...
# contrast: a vector representing the sample-variable contrast to use for extracting
#  effect on Qi; must be the same size as aux$L
...
# contrast: a vector representing the sample-variable contrast to use for extracting
#  effect on oi; must be the same size as aux$L
...
# contrast: a vector representing the sample-variable contrast to use for extracting
#  effect on Qi; must be the same size as aux$L
# include_O: Logical, indicating whether the effect of contrast on the global offset O
#  should be added to the differential expression matrix
...
# create a umap embedding for cells and their vector fields
...
# function for visualization of the vector field embedding
plot_vectorField <- function(embedding_mat,colour,alpha=1,randomize=T) {
  ...
  if(is.numeric(colour)) # the color variable is numeric
  {
    ...
    lim <- quantile(abs(embedding_obj$Color),0.99)
    ggplot(
      embedding_obj, aes_string( x="Dim1", y="Dim2", xend="To1", yend="To2", colour="Color", alpha="Alpha"))+
      # geom_point(size=0.05)+
      geom_segment(size=0.1,arrow = arrow(length=unit(0.1,units = "cm"),angle=15,type="closed"))+
...
# Function for plotting a 2D representation (embedding) of cells
plot_embedding <- function(embedding_mat,colour,randomize=T) {
  ...
  if(is.numeric(colour)) # the color variable is numeric
  {
    ...
    lim <- quantile(abs(embedding_obj$Var),0.99)
    ggplot(
      embedding_obj, aes_string( x="Dim1", y="Dim2", colour="Var"))+
      geom_point(size=0.05)+
      theme_minimal()+
      scale_color_gradientn( limits=c(-lim,lim), colours=c("blue","light grey","red"), oob=squish )
  } else {
